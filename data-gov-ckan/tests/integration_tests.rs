...
async fn test_basic_search() {
    ...
    assert!(
        ...
        "Should find climate-related datasets"
    );
    ...
    if let Some(ref results) = result.results {
        ...
        if let Some(first_dataset) = results.first() {
            assert!(
                first_dataset.title.is_some() || !first_dataset.name.is_empty(),
...
async fn test_pagination() {
    ...
    let first_page = client
        .package_search(Some("government"), Some(2), Some(0), None)
        .await
        .expect("First page should succeed");

    // Get second page
    let second_page = client
        .package_search(Some("government"), Some(2), Some(2), None)
        .await
...
async fn test_package_show() {
    ...
    let search = client
        .package_search(Some("energy"), Some(1), Some(0), None)
        .await
        .expect("Search should succeed");
...
async fn test_organization_list() {
    ...
    let result = client
        ...
        .await
        .expect("Organization list should succeed");
...
async fn test_dataset_autocomplete() {
    ...
    let result = client
        ...
        .await
        .expect("Dataset autocomplete should succeed");
    ...
    for dataset in &result {
        ...
        let contains_energy =
            name.to_lowercase().contains("energy") || title.to_lowercase().contains("energy");
        assert!(
            contains_energy,
            "Dataset '{}' or title '{}' should contain 'energy'",
            name, title
        );
    }
}
...
async fn test_tag_autocomplete() {
    ...
    let result = client
        ...
        .await
        .expect("Tag autocomplete should succeed");
...
async fn test_user_autocomplete() {
        ...
        .await
    {
        ...
        Err(CkanError::ApiError { status: 403, .. }) => {
            ...
            println!(
                "User autocomplete requires authentication (403 Forbidden) - this is expected"
            );
        }
...
async fn test_group_autocomplete() {
    ...
    let result = client
        ...
        .await
        .expect("Group autocomplete should succeed");
...
async fn test_organization_autocomplete() {
    ...
    let result = client
        ...
        .await
        .expect("Organization autocomplete should succeed");
...
async fn test_resource_format_autocomplete() {
    ...
    let result = client
        ...
        .await
        .expect("Resource format autocomplete should succeed");
    ...
    for format in &result {
        assert!(
            format.to_lowercase().contains("csv"),
            "Format '{}' should contain 'csv'",
            format
        );
    }
}
...
async fn test_error_handling() {
    ...
    match client
        .package_show("this-dataset-definitely-does-not-exist-12345")
        .await
    {
...
async fn test_large_result_handling() {
    ...
    let result = client
        ...
        .await
        .expect("Large result search should succeed");
    ...
    if let Some(results) = result.results {
        assert!(
            results.len() <= 100,
            "Server should limit results appropriately"
        );
    }
}
...
mod performance_tests {
    ...
    async fn test_search_performance() {
        ...
        let _result = client
            .package_search(Some("data"), Some(10), Some(0), None)
            .await
            .expect("Search should succeed");
